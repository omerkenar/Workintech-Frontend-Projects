...
describe('index.html', () => {
  ...
  it("css-12 projects-item class'ındaki resimlerin kuralları ayarlanmış", () => {
    expect(
      isPropertySetInCss(css, '.projects-sectionimg', 'width', '100%')
    ).toBe(true);
    expect(
      isPropertySetInCss(css, '.projects-sectionimg', 'aspect-ratio', '1/1')
    ).toBe(true);
    expect(
      isPropertySetInCss(css, '.projects-sectionimg', 'background', 'gray')
    ).toBe(true);
  });
